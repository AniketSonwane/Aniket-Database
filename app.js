...
async function getDriveItems(folderId) {
  ...
  const url = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&pageSize=1000&orderBy=folder,name&includeItemsFromAllDrives=true&supportsAllDrives=true&fields=${encodeURIComponent(fields)}&key=${encodeURIComponent(CONFIG.apiKey.trim())}`;
