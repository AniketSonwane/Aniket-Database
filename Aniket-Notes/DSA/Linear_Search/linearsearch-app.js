/**
 * Application Entry Point & Event Bindings for Linear Search
 * Default Array: [10, 25, 30, 45, 50], Default Target: 45
 */

document.addEventListener('DOMContentLoaded', () => {
  const defaultArray = [10, 25, 30, 45, 50];
  const defaultTarget = 45;
  const visualizer = new LinearSearchVisualizer();
  ...
  if (randomBtn) {
    randomBtn.addEventListener('click', () => {
      const size = Math.floor(Math.random() * 4) + 5; // 5 to 8 elements
      const randomArr = [];
      for (let k = 0; k < size; k++) {
        randomArr.push(Math.floor(Math.random() * 90) + 5);
      }
