/**
 * Application Entry Point & Event Bindings for Radix Sort
 * Default Array from User's C++ Code: [170, 45, 75, 90, 802, 24, 2, 66]
 */
  ...
  function parseInput() {
    ...
    for (let part of parts) {
      ...
      if (isNaN(parsed) || !Number.isInteger(parsed) || parsed < 0) {
        return null; // Radix Sort requires non-negative integers
      }
      nums.push(parsed);
    }
  ...
  if (randomBtn) {
    randomBtn.addEventListener('click', () => {
      ...
      for (let k = 0; k < size; k++) {
        randomArr.push(Math.floor(Math.random() * 950));
      }
